...
class LayeredDetectorMap(PfsDetectorMap):
    ...
    box : `Box`
        Bounding box for detector.
    spatial, spectral : `np.ndarray` of `float`
...
class OpticalModelDetectorMap(PfsDetectorMap):
    ...
    fiberMin : `float`
        Minimum fiber position (fiber units).
    ...
    def _writeImpl(self):
        ...
        header["HIERARCH pfs_fiberMin"] = self.fiberMin
        header["HIERARCH pfs_wavelengthDispersion"] = self.wavelengthDispersion
        header["HIERARCH pfs_wavelengthMin"] = self.wavelengthMin
